...
//  store->shard_name(prefix, cct->_conf->rgw_md_log_max_shards, hash_key, oid);
  ...
  return 0;//store->time_log_add(oid, now, section, key, bl);
}
...
int RGWMetadataLog::list_entries(void *handle,
  ...
  int ret = 0;//store->time_log_list(ctx->cur_oid, ctx->from_time, ctx->end_time,
                         ...
                         const string& start_marker, const string& end_marker)
{
  ...
  ret = 0;//store->time_log_trim(oid, from_time, end_time, start_marker, end_marker);
...
int RGWMetadataLog::lock_exclusive(int shard_id, utime_t& duration, string& zone_id, string& owner_id) {
  ...
  return 0;//store->lock_exclusive(store->zone.log_pool, oid, duration, zone_id, owner_id);
}
...
int RGWMetadataLog::unlock(int shard_id, string& zone_id, string& owner_id) {
  ...
  return 0;//store->unlock(store->zone.log_pool, oid, zone_id, owner_id);
}
...
int RGWMetadataManager::lock_exclusive(string& metadata_key, utime_t duration, string& owner_id) {
  ...
  return 0;//store->lock_exclusive(pool, oid, duration, zone_id, owner_id);  
}
...
int RGWMetadataManager::unlock(string& metadata_key, string& owner_id) {
  ...
  return 0;//store->unlock(pool, oid, zone_id, owner_id);  
}
...
int RGWMetadataManager::remove_entry(RGWMetadataHandler *handler, string& key, RGWObjVersionTracker *objv_tracker)
{
  ...
  ret = 0;//store->delete_system_obj(obj, objv_tracker);
  /* cascading ret into post_modify() */
                                  ...
                                  RGWObjVersionTracker *objv_tracker)
{
  ...
  ret = 0;//store->set_attrs(NULL, obj, attrs, rmattrs, objv_tracker);
  /* cascading ret into post_modify() */
