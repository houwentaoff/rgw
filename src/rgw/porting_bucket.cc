...
//  RGWObjectCtx obj_ctx(store);
  int ret = 0;//store->get_bucket_info(obj_ctx, bucket_name, bucket_info, NULL);
  if (ret < 0) {
...
void check_bad_user_bucket_mapping(RGWRados *store, const string& user_id, bool fix)
{
  ...
  size_t max_entries = 0;//cct->_conf->rgw_list_buckets_max_chunk;
...
//      ldout(store->ctx(), 0) << "failed to read user buckets: " << cpp_strerror(-ret) << dendl;
    ...
    for (map<string, RGWBucketEnt>::iterator i = buckets.begin();
      ...
      time_t mtime;
      RGWObjectCtx obj_ctx(store);
      int r = 0;//store->get_bucket_info(obj_ctx, bucket.name, bucket_info, &mtime);
      if (r < 0) {
...
//  rgw_obj obj(bucket, key);
  ...
  int ret = 0;//store->delete_obj(rctx, bucket_info, obj, bucket_info.versioning_status());
...
int RGWBucket::init(RGWRados *storage, RGWBucketAdminOpState& op_state)
{
  ...
  if (!bucket_name.empty()) {
    int r = 0;//store->get_bucket_info(obj_ctx, bucket_name, bucket_info, NULL);
    if (r < 0) {
...
int RGWBucket::link(RGWBucketAdminOpState& op_state, std::string *err_msg)
{
  ...
  RGWObjectCtx obj_ctx(store);
  int r = 0;//store->get_bucket_instance_info(obj_ctx, key, bucket_info, NULL, &attrs);
  if (r < 0) {
    return r;
  }
        ...
        list<rgw_obj_key>& objs_to_unlink, std::string *err_msg)
{
  ...
  if (fix_index) {
    int r = 0;//store->remove_objs_from_index(bucket, objs_to_unlink);
    if (r < 0) {
...
//  store->cls_obj_set_bucket_tag_timeout(bucket, BUCKET_TAG_TIMEOUT);
  ...
  while (is_truncated) {
    ...
    int r = 0;//store->cls_bucket_list(bucket, marker, prefix, 1000, true,
        ...
        std::string *err_msg)
{
  ...
  int r = 0;//store->bucket_check_index(bucket, &existing_stats, &calculated_stats);
  if (r < 0) {
...
int RGWBucket::get_policy(RGWBucketAdminOpState& op_state, ostream& o)
{
  ...
  RGWBucketInfo bucket_info;
  map<string, bufferlist> attrs;
  int ret = 0;//store->get_bucket_info(obj_ctx, bucket.name, bucket_info, NULL, &attrs);
  if (ret < 0) {
    return ret;
  }
...
static int bucket_stats(RGWRados *store, std::string&  bucket_name, Formatter *formatter)
{
  ...
  time_t mtime;
  RGWObjectCtx obj_ctx(store);
  int r = 0;//store->get_bucket_info(obj_ctx, bucket_name, bucket_info, &mtime);
  if (r < 0)
    return r;
  ...
  string bucket_ver, master_ver;
  string max_marker;
  int ret = 0;//store->get_bucket_stats(bucket, &bucket_ver, &master_ver, stats, &max_marker);
  if (ret < 0) {
...
//    store->time_log_prepare_entry(entry, ut, section, bucket.name, bl);
  ...
  for (miter = m.begin(); miter != m.end(); ++miter) {
    ...
    int ret = 0;//store->time_log_add(oids[miter->first], entries);
    if (ret < 0) {
...
int RGWDataChangesLog::list_entries(int shard, utime_t& start_time, utime_t& end_time, int max_entries,
  ...
  int ret = 0;//store->time_log_list(oids[shard], start_time, end_time,
...
int RGWDataChangesLog::get_info(int shard_id, RGWDataChangesLogInfo *info)
{
  ...
  int ret = 0;//store->time_log_info(oid, &header);
  if ((ret < 0) && (ret != -ENOENT))
    return ret;
                                    ...
                                    const string& start_marker, const string& end_marker)
{
  ...
  ret = 0;//store->time_log_trim(oids[shard_id], start_time, end_time, start_marker, end_marker);
                                    ...
                                    const string& start_marker, const string& end_marker)
{
  for (int shard = 0; shard < num_shards; shard++) {
    int ret = 0;//store->time_log_trim(oids[shard], start_time, end_time, start_marker, end_marker);
    if (ret == -ENOENT) {
      continue;
    }
    if (ret < 0)
...
public:
  ...
  int put(RGWRados *store, string& oid, RGWObjVersionTracker& objv_tracker,
    ...
    if (!exists || old_bci.info.bucket.bucket_id != bci.info.bucket.bucket_id) {
      ...
      rgw_bucket bucket;
      ret = 0;//store->set_bucket_location_by_rule(bci.info.placement_rule, oid, bucket);
      if (ret < 0) {
//        ldout(store->ctx(), 0) << "ERROR: select_bucket_placement() returned " << ret << dendl;
    ...
    ret = 0;//store->init_bucket_index(bci.info.bucket, bci.info.num_shards);
    if (ret < 0)
      return ret;
